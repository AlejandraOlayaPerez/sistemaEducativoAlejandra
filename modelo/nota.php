...
class nota{
    ...
    function consultarNota(){
        $oConexion=new conectar();
        $conexion=$oConexion->conexion();
        $sql="SELECT n.id, n.nota1, n.nota2, n.nota3, promedio FROM notas n INNER JOIN matricula m ON n.idMatricula=m.id WHERE m.idCurso=$this->idCurso AND m.idEstudiante=$this->idEstudiante";
        $result=mysqli_query($conexion,$sql);
